...
namespace SpellTracker
{
    class SpellTracker
    {
        ...
        {
            foreach (var ally in EntityManager.Heroes.Allies.Where(x => x.IsVisible && x.IsValid && !x.IsDead && x.IsMe))
            {
        ...
        {
            foreach (var ally in EntityManager.Heroes.Enemies.Where(x => x.IsVisible && x.IsValid && !x.IsDead))
            {
                for (var i = 0; i < Spells.Length; i++)
                {
                    _x = (int)ally.HPBarPosition.X + 30 * i;
